package com.java.javacore.admin;
...
public class ProjectStatistics {
    ...
    private static long calculateTestsInProject(){
        try {
            return Files.find(Paths.get(TEST_FOLDER_NAME), MAX_DEPTH_OF_PACKAGES, (path, attr) -> {
                return path.getFileName().toString().endsWith(JAVA_FILES_EXXTENSION);
            }).count();
        }catch (IOException e){
            e.printStackTrace();
            return 0;
